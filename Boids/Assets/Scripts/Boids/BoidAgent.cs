...
public class BoidAgent : MonoBehaviour
{
    ...
    public void ComputeAgentBehavior(int NNCount, int avoidCount, Vector3 alignmentMove, Vector3 avoidanceMove, Vector3 cohesionMove)
    {
        ...
        acceleration += ComputeAlignmentBehavior(NNCount, alignmentMove);
        if(NNCount != 0) acceleration += ComputeCohesionBehavior(cohesionMove);
        if(avoidCount != 0) acceleration += ComputeSeparationBehavior(avoidanceMove);

        acceleration += ComputeEnvironmentColisionsBehavior();
